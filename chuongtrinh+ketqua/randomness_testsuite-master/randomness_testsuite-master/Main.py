...
# Khoảng dòng 29 trong main.py
class Main(Frame):
    def __init__(self, master=None, batch_mode=False): # Thêm batch_mode=False
        Frame.__init__(self, master=master)
        self._master = master
        self._batch_mode = batch_mode  # Thêm dòng này để lưu trạng thái batch_mode
        self.init_variables()
        # init_window sẽ vẫn được gọi để khởi tạo các cấu trúc test cần thiết,
        # cửa sổ GUI sẽ được ẩn bởi script batch_runner.py
        self.init_window()
    ...
    def execute(self):
        ...
        if len(self.__binary_input.get_data().strip().rstrip()) == 0 and\
            ...
            if self._batch_mode:
                print("Warning: You must input the binary data or read the data from from the file.")
            else:
        ...
        try:
            ...
            if self._batch_mode:
                print("Execute: Test Complete.")  # Hoặc thông điệp tương ứng
            else:
    ...
    def save_result_to_file(self, output_path=None):  # Thêm tham số output_path=None
        print('Save to File')  # Giữ lại để theo dõi log

        data_source_type = None
        data_identifier = None
        test_data_header = ""

        if not len(self.__binary_input.get_data().strip().rstrip()) == 0:
            data_source_type = "direct_binary_input"
            # Lấy một phần dữ liệu nếu quá dài cho data_identifier
            binary_data_preview = self.__binary_input.get_data()[:50] + "..." if len(
                self.__binary_input.get_data()) > 50 else self.__binary_input.get_data()
            data_identifier = f"DirectInput_{binary_data_preview}"
            test_data_header = 'Test Data:' + self.__binary_input.get_data()
        elif not len(self.__binary_data_file_input.get_data().strip().rstrip()) == 0:
            data_source_type = "binary_file"
            data_identifier = self.__binary_data_file_input.get_data()  # Đây là đường dẫn tệp đầu vào
            test_data_header = 'Test Data File:' + data_identifier
        elif not len(self.__string_data_file_input.get_data().strip().rstrip()) == 0:
            data_source_type = "string_file"
            data_identifier = self.__string_data_file_input.get_data()
            test_data_header = 'Test Data File:' + data_identifier
        else:
            msg = "Warning: No data source found to save results for."
            if self._batch_mode:
                print(msg)
            else:
        ...
        if not self._test_result:
            msg = f"Warning: No test results available to save for {data_identifier}."
            if self._batch_mode:
                print(msg)
            else:
        ...
        actual_output_path = ""  # Sẽ lưu đường dẫn thực tế của tệp đã lưu

        if output_path:  # Chế độ batch hoặc đường dẫn đầu ra được xác định trước
            try:
                output_file_handle = open(output_path, 'w')
                actual_output_path = output_path
            except IOError as e:
                error_msg = f"Error opening output file {output_path}: {e}"
                if self._batch_mode:
                    print(error_msg)
                else:
                    messagebox.showerror("Error", error_msg)
                return
        else:  # Chế độ GUI, hỏi người dùng
            if self._batch_mode:  # Không nên xảy ra nếu output_path luôn được cung cấp ở chế độ batch
                print("Error: output_path not provided in batch mode for save_result_to_file.")
                return
            # Người dùng chọn nơi lưu tệp
            output_file_handle = asksaveasfile(mode='w', defaultextension=".txt")
            if output_file_handle:
                actual_output_path = output_file_handle.name  # Lấy đường dẫn từ đối tượng tệp

        if not output_file_handle:
            # Trường hợp này xử lý nếu asksaveasfile bị hủy hoặc open thất bại (mặc dù try-except ở trên nên bắt được)
            info_msg = "Save operation cancelled or failed."
            if self._batch_mode and not output_path:  # output_path không được cung cấp và không thể mở file
                print(info_msg + " (No output path specified for batch mode or file dialog cancelled)")
            elif not self._batch_mode:  # GUI mode cancellation
                messagebox.showinfo("Save", info_msg)
            # Nếu output_path được cung cấp nhưng open thất bại, lỗi đã được in ở trên
            return

        try:
            output_file_handle.write(test_data_header + '\n\n\n')
            # Giả sử kết quả mới nhất nằm ở vị trí 0 và được reset cho mỗi tệp
            if not self._test_result:  # Kiểm tra lại _test_result phòng trường hợp bị reset
                if self._batch_mode: print(f"Error: _test_result is empty before writing for {data_identifier}")
                return

            result_to_write = self._test_result[0]
            output_file_handle.write('%-50s\t%-20s\t%-10s\n' % ('Type of Test', 'P-Value', 'Conclusion'))
            self.write_result_to_file(output_file_handle, result_to_write)
        except Exception as e:
            error_msg = f"Error writing to file {actual_output_path}: {e}"
            if self._batch_mode:
                print(error_msg)
            else:
                messagebox.showerror("Error", error_msg)
        ...
        # Chỉ hiển thị thông báo thành công nếu actual_output_path có giá trị (tức là tệp đã được mở/tạo)
        if actual_output_path:
            success_msg = f"File saved successfully to {actual_output_path}."
            if self._batch_mode:
                print(success_msg)
            else:
